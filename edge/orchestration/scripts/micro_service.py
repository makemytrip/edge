...
class DCDeployMicroService(BaseOrchestration, StaggeredImplementation):
    ...
    def take_is(self, serverlist, datacenter, pool):
        status = True
        exception = None
        elk_logger_message = []
        ...
        try:
            ...
            response = self.lbobj.get_pool_is(is_servers, datacenter, pool, self.pool_servers_with_status)
    ...
    def take_oos(self, serverlist, datacenter, pool):
        status = True
        exception = None
        elk_logger_message = []
        try:
            ...
            response = self.lbobj.get_pool_oos(oos_servers, datacenter, pool, self.pool_servers_with_status)
            for server in response:
                status = response.get(server, False)
                if status is False:
                    self.failed_server_list.append(server)
    ...
    def deploy(self):
        try:
            ...
            datacenter = self.params.get('zone', None)
            pool = [pools.get('pool_name') for pools in self.params.get('pool_info', [])]
            node_detail = []
            self.all_servers_with_status = self.lbobj.get_pool_info("".join(pool), datacenter)
            for server, value in self.all_servers_with_status.items():
                    for ip, status in value.items():
                        server_status = True if status.get('enabled_status',None) in ['ENABLED_STATUS_ENABLED','ENABLED_STATUS_DISABLED_BY_PARENT'] else False
                        node_detail.append({'server' : ip.split(":")[0], 'enabled_status': server_status})
            self.all_servers_with_status = node_detail
            logger.info(self.all_servers_with_status)
            servers_with_status = []
            other_serverlist = []
            use_cache = False
    ...
    def start(self, datacenter, pool, serverlist):
        ...
        try:
            ...
            if self.is_hard is False:
                ...
                serverlist = self.take_is(serverlist, datacenter, pool)
        except Exception, e:
            logger.exception(e)
            status = False
            exception = str(e)
    ...
    def stop(self, datacenter, pool, serverlist):
        ...
        try:
            if self.is_hard is False:
                ...
                serverlist = self.take_oos(serverlist, datacenter, pool)
