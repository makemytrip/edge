...
class DCDeployDocker(BaseOrchestration, StaggeredImplementation):
    ...
    def get_task_details(cls, action):
        try:
            if action.lower() == "restart":
                ...
                return ["take_oos", "session_drain", "docker_stop", "docker_pull", "docker_run", "health_check", "take_is"]
    ...
    def install(self, datacenter, pool, serverlist):
        ...
        try:
            ...
            registry = self.dbobj.getValue('registry')
            registry_user = self.dbobj.getValue('registry_user')
            registry_password = self.dbobj.getValue('registry_password')

            docker_login_cmd = "docker login {registry} -u'{registry_user}' -p'{registry_password}'".format(registry=registry, registry_user=registry_user, registry_password=registry_password)
            serverlist = self.execute_command(serverlist, docker_login_cmd, "docker_login_cmd", False)

            # 2 Docker pull command
            docker_cmd = 'docker pull %s:%s' %(self.config.get('image_name'),self.config.get('version'))
            logger.debug("Docker pull command - %s" % docker_cmd)
            serverlist = self.execute_command(serverlist, docker_cmd, "docker_pull_cmd", False)
        ...
        try:
            ...
            if len(port) > 0:
                for key, val in port.items():
                    run_cmd = run_cmd+' '+'-p'+' '+key+':'+val
            volumes = self.config.get("volumes", {})
            if len(volumes) > 0:
                for key, val in volumes.items():
                    run_cmd = run_cmd+' '+'-v'+' '+key+':'+val
            env = self.config.get('env', {})
            if len(env) > 0:
                for key, val in env.items():
                    run_cmd = run_cmd+' '+'-e'+' '+key+'='+val
            run_cmd = run_cmd+' '+'--name'+' '+self.config.get('container_name')+' '+self.config.get('image_name')+':'+self.config.get('version')
    ...
    def stop(self, datacenter, pool, serverlist):
        ...
        try:
            ...
            # Call Stop Command
            check_container = "docker ps -aq --filter name='%s'"  % (self.config.get('container_name'))
            container_output = self.execute_command(serverlist, check_container, "check_container", False, do_return_stdout=True)
            logger.debug("%s container exists - %s, %s" % (serverlist, check_container, container_output))
            output = container_output.get('output', {})
                ...
                for cmd, val in result.items():
                    for i in val:
                        if type(i) is list:
                            container_output = i
            for server in serverlist:
                if len(container_output) > 0:
                    stop_command = 'docker stop {container} || true &&  docker rm -f {container} || true'.format(container=self.config.get('container_name'))
                    serverlist = self.execute_command(serverlist, stop_command, "stop_command")
                else:
                    logger.debug("container not found on %s" % (server))
            ...
            if status is False:
                raise Exception("No running process found for container - %s" % self.config.get('container_name'))
